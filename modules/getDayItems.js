...
export const getDayItems = (date) => {
    ...
    if (prevDays != 0) {
        for (let i = prevDays; i > 0; i--) {
            dayItems=[...dayItems, getDayItem('calendar__dayItem calendar__dayItem--prev', date, -i)];
        }
    } 
    ...
    for (let i = endDay; i + prevDays < 42; i++) {
        dayItems = [...dayItems, getDayItem('calendar__dayItem calendar__dayItem--next', date, i)];
    }
    return dayItems;
}
